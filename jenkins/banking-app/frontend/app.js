...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("new-deposit-btn")
    .addEventListener("click", () => openTransactionModal("deposit"));
  document
    .getElementById("new-withdrawal-btn")
    .addEventListener("click", () => openTransactionModal("withdrawal"));
  document
  ...
  document
    .getElementById("submit-account")
    .addEventListener("click", submitAccount);
        ...
        (txn) => `
            <div class="list-group-item">
                <div class="d-flex w-100 justify-content-between">
                    ...
                    <span class="fw-bold ${
                      txn.type === "deposit" ? "positive" : "negative"
                    }">
                        ${txn.type === "deposit" ? "+" : "-"}${formatCurrency(
          txn.amount
        )}
                    </span>
                </div>
...
function displayRecentTransactions(transactions) {
  ...
  recentTransactionsEl.innerHTML = transactions
    .map(
      (txn) => `
        <div class="mb-3">
        ...
        txn.amount
            ...
            <small class="text-muted">Account #${txn.account_id} • Status: ${
        txn.status
      }</small>
        </div>
    `
...
async function openTransactionModal(type) {
  ...
  document.getElementById("transaction-modal-title").textContent = `New ${
  ...
  try {
    ...
    accounts.forEach((account) => {
      ...
      option.textContent = `#${account.id} - ${account.owner} (${formatCurrency(
        account.balance
      )})`;
...
async function submitTransaction() {
  ...
  try {
    ...
    alert(
      `${type.charAt(0).toUpperCase() + type.slice(1)} processed successfully`
    );
  } catch (error) {
    console.error("Error creating transaction:", error);
    alert(
      "Failed to process transaction. Please check the console for details."
    );
  }
}
